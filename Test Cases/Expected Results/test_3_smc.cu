...
int matrixAdd(int block_size, int w, int h)
{
  ...
  float *h_C = (float *) malloc(mem_size);
    ...
    dim3 gridy(w / threads.x, h / threads.y);
...
matrixMulCUDA(float *C, float *A, float *B, int wA, int wB,dim3 __SMC_orgGridDim, int __SMC_workersNeeded, int *__SMC_workerCount, int * __SMC_newChunkSeq, int * __SMC_seqEnds)
{
	/*-------------|SMC Change: Add __SMC_Begin|-------------*/
	__SMC_Begin
...
int matrixMultiply(int argc, char **argv, int block_size, dim3 &dimsA, dim3 &dimsB)
{
    ...
    dim3 threads(block_size, block_size);
    /*-------------|SMC Change: Change Grid()|-------------*/
    dim3 __SMC_orgGridDim(dimsB.x / threads.x);

    /*-------------|SMC Change: Change Grid()|-------------*/
    dim3 __SMC_orgGridDim(dimsB.x / threads.x, dimsA.y / threads.y, 2);
    ...
    if (block_size == 16)
    {
        ...
        /*-------------|SMC Change: Add arguments to kernel function call|-------------*/
        matrixMulCUDA<16><<< gridx, threads >>>(d_C, d_A, d_B, dimsA.x, dimsB.x, __SMC_orgGridDim, __SMC_workersNeeded, __SMC_workerCount, __SMC_newChunkSeq, __SMC_seqEnds);
    }
    else
    {
        /*-------------|SMC Change: Add __SMC_init()|-------------*/
